...
execute if score .lag_clear_time nola.data matches 1 if score $lag_clear_messages nola.config matches 1 run tellraw @a [{"text":"In ","color":"gray"},{"text":"1 minute","color":"white"},{"text":" all unneeded entitys will be deleted.","color":"gray"}]
